...
from utils.schemas import APIResponse, LogContext, LogEvent #for each success data=obj -> meaing data returns an object and for each failure data is none!
async def get_user_intent(model, text: str) -> APIResponse: 
    log_info(
        LogContext(
            event=ServiceLog.AI_SERVICE_STARTED, #LogEvent's event takes enum obj dw, so need for .value
            function="get_user_intent"
        )
    )
    ...
    if not text or not text.strip():
        ...
        log_info(
            LogContext(
                event=ServiceLog.AI_SERVICE_FAILED,
                function="get_user_intent"
            )
        )
        ...
        log_info(
            LogContext(
                event=ServiceLog.EXITING_AI_SERVICE,
                function="get_user_intent"
            )
        )
    ...
    if deterministic_security_check(text):
        log_warning(
            LogContext(
                event=SecurityLog.PROMPT_INJECTION_DETECTED,
                function="get_user_intent"
                # request_id=request_id, #when u make front end, make sure that there is a way so this function gets request_id and user_id for ip banns
                # user_id=user_id
            )
        )
        log_info(
            LogContext(
                event=ServiceLog.AI_SERVICE_TERMINATED,
                function="get_user_intent"
            )
        )
        log_info(
    ...
    try:
        log_info(
            LogContext(
                event=ProviderLog.AI_PROVIDER_REQUEST,
                function="get_user_intent"
            )
        )
        ...
        log_info(
            LogContext(
                event=ProviderLog.AI_PROVIDER_IN_PROCESSING,
                function="get_user_intent"
            )
        )
        ...
        if check_provider_quota(e):
            log_exception(
                LogContext(
                    event=ServiceLog.AI_MY_QUOTA_REACHED,
                    function="get_user_intent",
                    exception=str(e),
            ...
            log_exception(
                LogContext(
                    event=ProviderLog.AI_PROVIDER_FAILURE,
                    function="get_user_intent",
                    exception=str(e),
                    exception_type=type(e).__name__
                )
    ...
    log_info(
        LogContext(
            event=ProviderLog.AI_PROVIDER_SUCCESS,
            function="get_user_intent",
        )
    )
    ...
    if extracted_parsed is not None:
        if extracted_parsed.intent == "malicious_injection":
            log_warning( #injection is warning coz 5 warning form same user and he bann gng 
                LogContext(
                    event=SecurityLog.PROMPT_INJECTION_DETECTED,
                    function="get_user_intent"
                )
            ...
            log_info(
                LogContext(
                    event=SecurityLog.UNKNOWN_INPUT,
                    function="get_user_intent"
                )
            )    
            log_info(
            ...
            log_info(
                LogContext(
                    event=SecurityLog.INAPPROPRIATE_CONTENT_DETECTED,
                    function="get_user_intent"
                )
            )
            log_info(
            ...
            log_info(
                LogContext(
                    event=ServiceLog.AI_SERVICE_COMPLETED,
                    function="get_user_intent"
                )
            )  
            log_info(
                LogContext(
                    event=ServiceLog.AI_SERVICE_ENDED,
                    function="get_user_intent"
                )
            ) 
            log_info(
    ...
    if extracted_parsed is None:
        log_info(
            LogContext(
                event=RepairLog.AI_REPAIR_INITIALIZED,
                function="get_user_intent"
            )
        )    
        r"""
    ...
    if raw is None:
        ...
        log_info(
            LogContext(
                event=RepairLog.AI_REPAIR_INITIALIZATION_STOPPED,
                function="get_user_intent"
            )
        )
        log_info(
    ...
    try:    
        log_info(
            LogContext(
                event=RepairLog.AI_REPAIR_STARTED,
                function="get_user_intent"
            )
        )  
        log_info(
            LogContext(
                event=RepairLog.AI_REPAIR_IN_PROGRESS,
                function="get_user_intent"
            )
        ) 
        ...
        if check_provider_quota(e):
            ...
            log_info(
                LogContext(
                    event=RepairLog.AI_REPAIR_PREMATURELY_ENDED,
                    function="get_user_intent"
                )
            )    
            log_info(
            ...
            log_exception(
                LogContext(
                    event=RepairLog.AI_REPAIR_PREMATURELY_ENDED,
                    function="get_user_intent",
                    exception=str(e), #why dont u use service failed first? well, look ive exception here, telling what caused repair fail
                    exception_type=type(e).__name__
                )
            )
    ...
    if recovered is None:
        log_info(
            LogContext(
                event=RepairLog.AI_REPAIR_FAILED,
                function="get_user_intent"
            )
        )
        log_info(
        ...
        log_info(
            LogContext(
                event=RepairLog.AI_REPAIR_SUCCESS,
                function="get_user_intent"
            )
        )
        log_info(
